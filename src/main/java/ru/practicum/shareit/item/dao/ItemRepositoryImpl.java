...
public class ItemRepositoryImpl implements ItemRepositoryCustom {
    ...
    public ItemRepositoryImpl(@Lazy BookingRepository bookingRepository, @Lazy CommentRepository commentRepository, @Lazy ItemRepository itemRepository) {
    ...
    public ItemDto getByIdForResponse(long userId, long id) {
        ItemDto itemDto = ItemMapper.toItemDto(itemRepository.findById(id).orElseThrow(() -> new ObjectNotFoundException("Не найден предмет " + id)));
    ...
    private List<BookingForItemDto> getBooking(long itemId) {
        return bookingRepository.findAllByItem_IdAndStatus(itemId, BookingStatus.APPROVED);
    }
}
